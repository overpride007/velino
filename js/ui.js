/**
 * ui.js - Funções relacionadas à interface do usuário
...
/**
 * Controla a notificação de uso da página
 */
document.addEventListener('DOMContentLoaded', function() {
    // Inicializa os botões da notificação de uso
    const closeNotificationBtn = document.getElementById('closeNotification');
    const quickStartBtn = document.getElementById('quickStartBtn');
    
    if (closeNotificationBtn) {
        closeNotificationBtn.addEventListener('click', function() {
            document.getElementById('usageNotification').style.display = 'none';
        });
    }
    ...
    if (quickStartBtn) {
        quickStartBtn.addEventListener('click', function() {
            document.getElementById('usageNotification').style.display = 'none';
            const guia = document.getElementById('guia-iniciantes');
            if (guia) {
...
/**
 * Inicialização da página e verificação de autenticação
 */
document.addEventListener('DOMContentLoaded', function() {
    // Verificar autenticação (função definida em auth.js)
    if (typeof verificarAutenticacao === 'function') {
        const authStatus = verificarAutenticacao();
        
        // Controlar exibição com base no status de autenticação
        if (authStatus.status === 'authenticated') {
            // Usuário autenticado - mostrar conteúdo principal
            document.getElementById('tela-bloqueio').style.display = 'none';
            document.getElementById('conteudo-principal').style.display = 'block';
        } else {
            // Usuário não autenticado - mostrar tela de bloqueio
