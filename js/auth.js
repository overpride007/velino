...
function verificarAutenticacao() {
    ...
    const dominioCorreto = window.location.hostname === 'overpridex.up.railway.app';
    // Não verificar mais a porta
    
    // Se não estamos no domínio correto, considerar como não autenticado
    if (!dominioCorreto) {
        // Verificar se temos acesso PF ativo
        const pfKey = localStorage.getItem('pf_access_key');
        const pfTimestamp = localStorage.getItem('pf_access_timestamp');
        
        if (pfKey && pfTimestamp && (agora - parseInt(pfTimestamp)) < 86400000) {
