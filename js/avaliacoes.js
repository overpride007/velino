...
// Dados de avaliações (simulados inicialmente)
const dadosAvaliacoes = {
    // ID da extensão: { media: número, total: número }
    'manhastro': { media: 4.5, total: 28 },
    'manga-livre': { media: 4.3, total: 15 },
    'sussy-toons': { media: 4.2, total: 12 },
    'manga-terra': { media: 4.0, total: 20 },
    'manga-br': { media: 3.8, total: 18 },
    'leitor-manga': { media: 3.9, total: 22 },
...
function inicializarAvaliacoes() {
    // Carregar avaliações reais do servidor (quando disponível)
    // carregarAvaliacoes();
...
// Função para carregar avaliações do servidor (para implementação futura)
async function carregarAvaliacoes() {
    try {
        const response = await fetch(`${avaliacoesConfig.apiBaseUrl}/api/avaliacoes`);
        if (!response.ok) {
            throw new Error(`Erro ${response.status}`);
        }
        const avaliacoes = await response.json();
        // Atualizar dados de avaliações
...
// Função para exibir avaliações na seção Recomendações do Mês
...
function criarElementoAvaliacao(media, total) {
    ...
    // Adicionar texto com a média e total de avaliações
    const textoAvaliacao = document.createElement('div');
    textoAvaliacao.className = 'text-xs text-gray-300 mt-1';
    textoAvaliacao.textContent = `${media.toFixed(1)} (${total} avaliações)`;
...
function gerarEstrelas(avaliacao) {
    ...
    for (let i = 1; i <= estrelasInteiras; i++) {
        estrelas += '<i class="fas fa-star text-yellow-400" style="color: #FFD700; text-shadow: 0 0 5px rgba(255, 215, 0, 0.5);"></i>';
    }
